...
DWORD WINAPI RecvAndSendMessageThread(void* param) {
    ...
    while (true) {
        ...
        if (CheckError(&header, sizeof(header)) == false || header.seq != NextGroup) {
            cout << "[server]:分组" << NextGroup << "的数据报接收失败！接收seq:" << header.seq << endl;
            // 发送前一个Seq的Ack
            header.reset();
            header.flag = ACK;
            header.ack = NextGroup-1; // 确认收到分组
            // 计算校验位
            PseudoHeader* Phead = new PseudoHeader();
            header.Checksum = Phead->Cal_Checksum(&header);
            memcpy(SendBuff, &header, sizeof(header));
        ...
        cout << "[server]:分组" << NextGroup << "发来的数据报[" << pckNum++ << "]接收成功！" <<NextGroup<< endl;
        ...
        // 接收端延时（一般都是速度：传>收），其实在处理的过程中也有一定的延时
        // Sleep(timmer);

        header.reset();
        header.flag = ACK;
        header.ack = NextGroup; // 确认收到分组
        // 计算校验位
        PseudoHeader* Phead = new PseudoHeader();
        header.Checksum = Phead->Cal_Checksum(&header);
        memcpy(SendBuff, &header, sizeof(header));
        ...
        // 等待下一次
        NextGroup++;
        cout << "[server]:等待分组" << NextGroup << "发来的数据......" << endl;
    }
